...
module.exports = (function () {
  ...
  function updateDict(info, dict) {
    ...
    var indentLength = indent.prefixLength + indent.extra;
    var infoLength = info.prefixLength + info.extra;
  ...
  return {
      ...
      ast.traverseByType('block', function (node) {
        ...
        walk({
          ...
          payload: function payload(info, i) {
            ...
            var content = node.get(i).get(x - 1).content;
            var updatedIndent = updateIndent(info, dict, content);
        ...
        walk({
          ...
          payload: function payload(info, i) {
            ...
            var content = whitespaceNode.content;
            var updatedContent = updateIndent(info, dict, content);
    ...
    detect: function detect(ast) {
      ...
      ast.traverseByType('block', function (node) {
        ...
        var getResult = function getResult(options) {
        ...
        walk({
          ...
          payload: function payload(info, i) {
            if (node.get(i - 1) && node.get(i - 1).content) {
              var nodeLength = node.get(i - 1).content.replace(/^[ \t]*\n+/, '').length;
        ...
        walk({
          ...
          payload: function payload(info, i) {
            ...
            if (node.get(i).get(x - 1)) {
              var nodeLength = node.get(i).get(x - 1).content.replace(/^[ \t]*\n+/, '').length;
              var sum = nodeLength + info.prefixLength;
              getResult({ node: node, sum: sum, info: info, i: i });
            }
          }
